...
const policyObjs = map(
  ...
  (policyArr): PolicyObj => {
    return {
      min: parseInt(split(policyArr[0], "-")[0]),
      max: parseInt(split(policyArr[0], "-")[1]),
...
const result = filter(
  map(policyObjs, (policyObj) =>
    filter(
      split(policyObj.password, ""),
      (letter) => letter === policyObj.letter
    ).length >= policyObj.min &&
    filter(
      split(policyObj.password, ""),
      (letter) => letter === policyObj.letter
    ).length <= policyObj.max
      ? 1
      : 0
  ),
  (count) => count > 0
).length;
// const result = map(policyObjs, (policyObj) => (filter(split(policyObj.password, ''), (letter) => letter === policyObj.letter).length >= policyObj.min && filter(split(policyObj.password, ''), (letter) => letter === policyObj.letter).length <= policyObj.max) ? 1 as number : 0 as number).reduce((sum, count) => sum = sum + count)
